...
type SeriesRecord = CommonRecord & { category: "series"; titleEnglish: string; posterUrl: string; tmdbId?: string; seasons: Season[]; networkName?: string };
...
type FormState = { title: string; author: string; publisher: string; coverUrl: string; titleEnglish: string; releaseYear: string; directorNames: string; posterUrl: string; city: string; museum: string; exhibition: string; seasons: Season[]; tmdbId: string; networkName: string; isbn: string; externalSource: string; externalId: string };

const STORAGE_KEY = "archive-records-v1";
const archiveYears = Array.from({ length: 100 }, (_, index) => 2000 + index);
...
const emptyForm: FormState = { title: "", author: "", publisher: "", coverUrl: "", titleEnglish: "", releaseYear: "", directorNames: "", posterUrl: "", city: "", museum: "", exhibition: "", seasons: [], tmdbId: "", networkName: "", isbn: "", externalSource: "", externalId: "" };
...
export default function Home() {
  ...
  useEffect(() => { try { setRecords(JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "[]") as ArchiveRecord[]); } catch { setRecords([]); } }, []);
  ...
  function persist(nextRecords: ArchiveRecord[]) { setRecords(nextRecords); }
  ...
  function openEdit(record: ArchiveRecord) {
    ...
    if (record.category === "series") setForm({ ...emptyForm, titleEnglish: record.titleEnglish, posterUrl: record.posterUrl, tmdbId: record.tmdbId ?? "", networkName: record.networkName ?? "", seasons: record.seasons });
  ...
  async function chooseResult(result: Result) {
    ...
    try {
      ...
      if (category === "movie") { const details = await getMovieDetails(result.id); setForm({ ...emptyForm, ...details, titleEnglish: details.titleEnglish || result.title || "" }); }
      if (category === "series") { const details = await getSeriesDetails(result.id); setForm({ ...emptyForm, ...details, titleEnglish: details.titleEnglish || result.title || "" }); }
      setDialogMode("form");
  ...
  function saveRecord() {
    ...
    else if (category === "series") next = { ...base, category: "series", titleEnglish: form.titleEnglish.trim(), posterUrl: form.posterUrl.trim(), tmdbId: form.tmdbId, networkName: form.networkName.trim(), seasons: form.seasons.map((season) => ({ ...season })) };
  ...
  return <main className="archive-app">
    <header className="archive-header"><h1>ARCHIVE</h1><div className="archive-year-control"><select aria-label="Archive year" value={selectedYear} onChange={(event) => setSelectedYear(Number(event.target.value))}>{archiveYears.map((year) => <option key={year} value={year}>{year}</option>)}</select></div></header>
    ...
    <section className="archive-list" aria-live="polite">{visibleRecords.length === 0 ? <div className="archive-empty"><p>No {category === "gallery" ? "gallery visits" : `${category}s`} yet.</p><span>Tap + to add one.</span></div> : visibleRecords.map((record) => <article className="archive-row" key={record.id} onClick={() => openEdit(record)}>
      ...
      {record.category === "series" && <>{imageOrPlaceholder(record.posterUrl, record.titleEnglish)}<div className="archive-row-copy archive-series-copy"><h2>{record.titleEnglish}</h2>{record.networkName && <p className="archive-series-network">{record.networkName}</p>}<div className="season-strip" onClick={(event) => event.stopPropagation()}>{record.seasons.length ? record.seasons.map((season) => <button key={season.seasonNumber} className={season.watched ? "watched" : ""} aria-label={`Season ${season.seasonNumber} ${season.watched ? "watched" : "unwatched"}`} onClick={() => toggleSeason(record.id, season.seasonNumber)}>{season.watched ? "■" : "□"}</button>) : <span>No seasons</span>}</div></div></>}
      {record.category === "gallery" && <div className="archive-row-copy"><h2>{record.museum} · {record.city}</h2>{record.exhibition && <p>{record.exhibition}</p>}</div>}
    </article>)}</section>
